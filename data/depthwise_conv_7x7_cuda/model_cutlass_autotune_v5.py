"""Vertically fused, vector-pipelined CUTLASS depthwise 7x7 implementation.

The FP16 path computes an 8x128 output tile, uses 16-byte asynchronous input
copies, specialized warp shuffles, prefetched register weights, and half2
output stores.
"""
...
__device__ __forceinline__ void load_stage(
    ...
    const __half* input, int64_t plane, int tile_y, int tile_x,
    int height, int width) {
  const int tid = threadIdx.x;
  constexpr int VECTORS_PER_ROW = 16;
  ...
  // Fill logical x=-1..2 and x=131..133.
  ...
  if (tid < ROWS_PER_STAGE * SCALARS_PER_ROW) {
    ...
    const int sx = item < 4 ? item - 1 : 131 + item - 4;
    const int gy = tile_y + 4 * y_group + 2 * stage + row - R;
    const int gx = tile_x + sx - R;
...
__device__ __forceinline__ void load_weight_fragment(
    Mma::FragmentB& bf, const __half* packed_weight, int channel, int stage,
    int group, int thread) {
  const __half* ptr = packed_weight + channel * M * K + group * K +
                      stage * BLOCK_K + 2 * thread;
  const uint32_t pair0 = reinterpret_cast<const uint32_t*>(ptr)[0];
  const uint32_t pair1 = reinterpret_cast<const uint32_t*>(ptr + 8)[0];
  bf[0] = cutlass::half_t::bitcast(static_cast<uint16_t>(pair0));
  bf[1] = cutlass::half_t::bitcast(static_cast<uint16_t>(pair0 >> 16));
  bf[2] = cutlass::half_t::bitcast(static_cast<uint16_t>(pair1));
  bf[3] = cutlass::half_t::bitcast(static_cast<uint16_t>(pair1 >> 16));
}
...
__global__ void cutlass_register_kernel(
  ...
  const int64_t plane = (static_cast<int64_t>(batch) * gridDim.y + channel) *
                        height * width;
  const bool interior = tile_x >= R && tile_x + OUT_W + R <= width &&
  ...
  if (interior)
    load_stage<true>(input_stage, 0, 0, input, plane, tile_y, tile_x,
                     height, width);
  else
    load_stage<false>(input_stage, 0, 0, input, plane, tile_y, tile_x,
                      height, width);
  copy_async_wait();
  __syncthreads();
...
#pragma unroll
  ...
  Mma::FragmentB weight_fragments[NUM_STAGES];
  load_weight_fragment(weight_fragments[0], packed_weight, channel, 0,
                       group, thread);
...
#pragma unroll
  for (int stage = 0; stage < K_STAGES; ++stage) {
    ...
    if (stage + 1 < K_STAGES) {
      if (interior)
        load_stage<true>(input_stage, read_buffer ^ 1, stage + 1, input,
                         plane, tile_y, tile_x, height, width);
      else
        load_stage<false>(input_stage, read_buffer ^ 1, stage + 1, input,
                          plane, tile_y, tile_x, height, width);
      load_weight_fragment(weight_fragments[read_buffer ^ 1], packed_weight,
                           channel, stage + 1, group, thread);
    }
...
#pragma unroll
      for (int nt = 0; nt < N_TILES_PER_WARP; ++nt) {
        ...
        mma(accum[yg][nt], af, weight_fragments[read_buffer],
            accum[yg][nt]);
      }
    }
    if (stage + 1 < K_STAGES) {
